...
import { MONGO_DB, MONGO_URI } from '../config/database';
import {
  ActivityModel,
...
const seed = async (): Promise<void> => {
  ...
  const leaderboard = await LeaderboardEntryModel.insertMany([
  ...
  console.log(`Seeded ${MONGO_DB}: ${users.length} users, ${teams.length} teams, ${activities.length} activities, ${leaderboard.length} leaderboard entries, ${workouts.length} workouts.`);
};
